...
#
#     # --- ADD TIME-DEPENDENT DECAY TO PUMPING ---
#     Gamma_init = float(params['gamma'])
#     Tp = 60.0  # Matches the ~60s timeframe of the experimental decay window
#     Gamma_t = Gamma_init * np.exp(-t / Tp)
#
#     # Use Gamma_t instead of Gamma in the equations below
#     dd1_dt = Gamma_t * (d1_0 - d1) - (d1 / T1) - (4 * coupling_beta) * (a1 ** 2 + a1 * a2 * np.cos(phi2 - phi1))
#     dd2_dt = Gamma_t * (d2_0 - d2) - (d2 / T1) - (4 * coupling_beta) * (a2 ** 2 + a2 * a1 * np.cos(phi2 - phi1))
#
...
def system(t, y):
    ...
    # --- MAKE PUMPING TIME-DEPENDENT ---
    Gamma_init = float(params['gamma'])
    Tp = 60.0  # Decay time constant matching your Mathematica notebook
    Gamma_t = Gamma_init * np.exp(-t / Tp)

    # Use Gamma_t instead of Gamma in your differential equations
    dd1_dt = Gamma_t * (d1_0 - d1) - (d1 / T1) - (4 * coupling_beta) * (a1**2 + a1 * a2 * np.cos(phi2 - phi1))
    dd2_dt = Gamma_t * (d2_0 - d2) - (d2 / T1) - (4 * coupling_beta) * (a2**2 + a2 * a1 * np.cos(phi2 - phi1))
...
# Sort frequencies so negative and positive lines connect smoothly
sort_idx = np.argsort(freq_full)
freq_full = freq_full[sort_idx]
Y_full = Y_full[sort_idx]

# Create a window mask around 0 Hz instead of cutting out negatives
view_mask = (freq_full >= -15) & (freq_full <= 15)
...
def update_plots(relayoutData, run_notes):
    ...
    fig_freq = go.Figure()
    fig_freq.add_trace(
        go.Scatter(x=freq_full[view_mask], y=Y_full[view_mask], name="Full FFT", line=dict(color='lightgrey')))

    if len(indices) > 1:
        f_z = np.fft.fftfreq(len(indices), d=(t[1] - t[0]))
        Y_z = np.abs(np.fft.fft(output_signal[indices]))

        # Sort Zoomed FFT arrays
        sort_z = np.argsort(f_z)
        f_z = f_z[sort_z]
        Y_z = Y_z[sort_z]

        zoom_mask = (f_z >= -15) & (f_z <= 15)
        fig_freq.add_trace(
            go.Scatter(x=f_z[zoom_mask], y=Y_z[zoom_mask], name="Zoomed FFT", line=dict(color='red', dash='dash')))
    ...
    fig_freq.update_layout(
        ...
        xaxis_range=[-15, 15],  # Exposes both your negative and positive peaks!
        height=700,
